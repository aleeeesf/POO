...
Cadena::Cadena(const Cadena& cdn)
{
	...
	for(int i = 0; i<tam_; i++)	s_[i] = cdn[i];	//Copiamos todos los elementos de cdn
												// a la nueva cadena
	s_[tam_] = '\0';
...
Cadena::Cadena(const char *str)
{
	...
	for(int i = 0; i<tam_; i++) s_[i] = str[i];	//Copiamos la cadena a s_
...
/*Devuelve posición de la cadena*/
...
Cadena Cadena::substr(int ind_,const int& num_)const
{
	...
	if(ind_ > tam_ || num_ < 0 || ind_ < 0) 	//Comprobamos que el índice y el tamaño introducidos sean validos
